 ...
 */
Array.prototype.sample = function (replacement = true) {
  // random index
...
const main = async () => {
  ...
  for (let id = 1; id <= totalSupply; id++) {
    ...
    const { race, character, ideology } = candidates.sample(false);

    // assemble metadata
    const creature = {
      name: names.sample(false),
      description: `${Articles.articlize(
        character
      ).capitalize()} ${ideology} ${race} called ${names[id - 1]}.`,
